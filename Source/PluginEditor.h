...
private:
    ...
    // same tiling), stores both as editor state, then applies the compact
    // 16-bar loop via applyGeneratedLoop(). Drums+bass+melody+pad all become
    // audible from bar 1 of one immediately-looping 16-bar idea that plays
    // the whole DROP->BREAKDOWN->PRE_DROP->DROP arc every repeat - see
    // Engine::generateCompactLoop's own header comment
    // (Source/Engine/BreakdownArrangement.h) for why the long-form
    // Arrangement/MusicState machinery, and the old separate DROP/BREAKDOWN
    // toggle, are no longer what this button drives.
    void generateLoopClicked();

    // Sends the current identity's stitched Engine::CompactLoop (see
    // generateLoopClicked()) to setGeneratedDrumPattern/
    // setGeneratedMelodyPattern - the actual DROP/BREAKDOWN/PRE_DROP
    // contrast now lives directly in the pattern data itself (bars 8-15
    // are the breakdown span, baked in at generation time), not in a
    // separate mode-driven mute/gain overlay - manual per-role mute
    ...
    // std::nullopt until the first Generate click. Currently UNUSED by
    // generateLoopClicked()/applyGeneratedLoop() - the "simplify to a
    // flat, looping 8-bar groove" phase wires the Generate button to
    // currentGrooveLoop below instead (see Engine/GrooveLoop.h). Left
    // declared, not deleted, so the 16-bar loop-generator/arrangement
    // path can be revisited later without rebuilding this member -
    // MusicIdentity.h/BreakdownArrangement.h/generateCompactLoop are all
    // still compiled and still pass their own tests, just not called
    // from here right now.
    std::optional<Engine::MusicIdentity> currentIdentity;

    // The flat, non-arc, non-arrangement 8-bar groove generated by the
    // last generateLoopClicked() - what applyGeneratedLoop() actually
    // uses now. std::nullopt until the first Generate click.
    std::optional<Engine::GrooveLoop> currentGrooveLoop;
    // The bpm that specific GrooveLoop was generated at - GrooveLoopParams/
    // GrooveLoop don't carry bpm themselves (Engine::generateGrooveLoop has
    // no bpm-dependent logic), but sample selection (Engine::scoreForRole's
    // bpmHint) and the drum-stem-export status block still need a real,
    // non-live-transport-dependent bpm value to report - same role
    // currentIdentity->bpm used to serve.
