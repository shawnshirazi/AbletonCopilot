...
private:
    ...
    // Drums/Bass/Melody are three INDEPENDENT generation actions, each
    // with its own freshly-drawn seed and its own stored state
    // (currentDrumGroove/currentBassMotif/currentMelodyMotif) - regenerating
    // one never touches the others' stored data or the processor state
    // that drives them. This is the fix for the generation-coupling bug
    // documented in the plan this was built from: Engine::GrooveLoop used
    // to generate bass internally (one seed, one struct, drums+bass
    // inseparable); it is now drum-only (Source/Engine/GrooveLoop.h), and
    // bass is generated directly here via Engine::generateBassPattern
    // (BassEngine.h, unmodified, already a self-contained 8-bar generator).
    void generateDrumsClicked();
    void generateBassClicked();
    void generateMelodyClicked();

    // Data-only halves of the three functions above (fresh seed -> real
    // generator call -> processor.setGenerated*Pattern) with NO display
    // refresh - factored out purely so generateLoopClicked() (below) can
    // call all three once each and refresh the display ONE time at the
    // end, instead of three redundant grid/resized() rebuilds for a
    // single button click. Each *Clicked() function above is just
    // `generateXData(); refreshLoopDisplay();`.
    void generateDrumsData();
    void generateBassData();
    void generateMelodyData();

    // Refreshes currentLoopBpm from the live host tempo (or the 124.0
    // fallback) - called at the top of every one of the four *Clicked()
    // functions above, since BPM is shared across all categories, not
    // owned by any single one.
    void refreshLoopBpm();

    // The single top-level "Generate" button (generateLoopButton) - a thin
    // convenience wrapper: draws three independent seeds and calls the
    // three functions above in sequence, then refreshLoopDisplay() ONCE at
    // the end (not three times) - "one click, hear a full loop" is
    // preserved, but the underlying state is genuinely three independent
    // slots underneath, not one coupled generation.
    ...
    // currentMelodyMotif (each independently, only the ones that actually
    // exist) - a pure display refresh, never regenerates anything, never
    // overwrites stored data. Called at the end of every one of the three
    // generate-click handlers above (and generateLoopClicked) so the grid
    // always reflects the real combined current state without any single
    // action needing to know about, or touch, the other two categories'
    // data. Pad (track 2) has no generator this phase (see GrooveLoop.h) -
    // always displayed/sent as an explicit all-off pattern.
    void refreshLoopDisplay();
    ...
    // The identity/melody generated by the OLD single-click 16-bar
    // loop-generator workflow - std::nullopt, currently UNUSED. The
    // "simplify to a flat, looping 8-bar groove" phase wires the Generate
    // button(s) to currentDrumGroove/currentBassMotif/currentMelodyMotif
    // below instead (see Engine/GrooveLoop.h). Left declared, not deleted,
    // so the 16-bar loop-generator/arrangement path can be revisited later
    // without rebuilding this member - MusicIdentity.h/
    // BreakdownArrangement.h/generateCompactLoop are all still compiled
    // and still pass their own tests, just not called from here right now.
    std::optional<Engine::MusicIdentity> currentIdentity;

    // Three INDEPENDENT stored-state slots, one per generation category -
    // regenerating one (generateDrumsClicked/generateBassClicked/
    // generateMelodyClicked) only ever touches its own slot.
    // std::nullopt/empty until that category's own first Generate click.
    // Engine::GrooveLoop is drum-ONLY now (bass was split out - see
    // GrooveLoop.h's own header comment for why).
    std::optional<Engine::GrooveLoop> currentDrumGroove;
    std::vector<int8_t>                currentBassMotif;   // 128 steps once generated, Engine::kBassOffValue = silence
    std::vector<int8_t>                currentMelodyMotif; // 128 steps once generated (one 8-bar motif, not tiled x2)

    // WHICH sample file plays each drum role - stored (not just computed
    // inline and discarded) so refreshLoopDisplay() can rebuild the
    // drum-sample diagnostic text without re-running selectDrumSamples()
    // (which draws its own random seed) on every bass/melody regeneration.
    // Re-running sample selection on a bass-only Generate click would
    // silently change which drum SAMPLES play even though the drum
    // PATTERN didn't - exactly the kind of "Generate Bass overwrote
    // drums" the independent-generation fix is about. Only
    // generateDrumsData() ever writes this.
    DrumSampleSelection currentDrumSampleSelection;

    // The bpm the current loop was generated at - GrooveLoopParams/
    // GrooveLoop/BassPatternParams don't carry bpm themselves (none of the
    // generators are bpm-dependent), but sample selection
    // (Engine::scoreForRole's bpmHint) and the drum-stem-export status
    // block still need a real, non-live-transport-dependent bpm value to
    // report - same role currentIdentity->bpm used to serve. Shared across
    // all three categories (per the "share the same BPM" requirement) -
    // set once per Generate click, not per category.
    double currentLoopBpm = 124.0;
    ...
    // The structured DRUMS/BASS/MELODY/PAD/SECTION block (see
    // RuntimeStatusText.h) - a compact, honest "what's actually loaded and
    // playing right now" summary shown above the detailed per-role sample
    // diagnostics (drumPatternStatusLabel), which stays as-is (not
    // removed - "don't remove useful diagnostic information"). Filled in
    // per-Generate-click by applyGeneratedLoop() except the SECTION line,
    // which is refreshed live every timerCallback() tick from the real
    // host playhead position.
    juce::Label       structuredStatusLabel;
    juce::String      structuredStatusPrefix; // everything except the SECTION line - see RuntimeStatusText::buildStatusPrefix
    ...
    // Same independent-mute-only contract as drumRoleMutes, for the 3
    // melody voices (Bass=0/Melody=1/Pad=2) - calls
    // PluginProcessor::setMelodyTrackMuted, which (like
    // setGeneratedDrumRoleMuted) only ever gates triggering, never
